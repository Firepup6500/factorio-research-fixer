...
-- This seems to be the vanilla game's fault? circuit-network claims to require logisitic science but seemingly doesn't???
  ...
  if tech_data.unit and tech_data.unit.ingredients then
    for _, ingredient in ipairs(tech_data.unit.ingredients) do
      ...
      if required_tech and data.raw.technology[required_tech] then
        if debug then log("DEBUG: " .. tech_name .. " has prereqs " .. (tech_data.prerequisites and table.concat(tech_data.prerequisites, ", ") or "NONE")) end
        if not has_prerequisite_in_chain(tech_name, required_tech) then
          ...
          log("Modified: " .. tech_name .. " now requires " .. required_tech)
